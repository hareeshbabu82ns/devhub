...
const sanscriptUtilsAPI = axios.create({
  baseURL:
    process.env.SANSCRIPT_UTILS_URL ||
    "https://sanskrit-utils.kube.terabits.io/graphql",
  // timeout: 5000,
...
module.exports = {
  dictionaryBrowse: async ({ browseWith }, requestedFields) => {
    const query = `
    ...
    `;
    const res = await sanscriptUtilsAPI.post("", {
      query,
      variables: { browseWith },
    });
    // console.log( res?.data )
    return res?.data?.data?.dictionaryBrowse;
  },
  ...
  splits: async ({ text, schemeFrom, schemeTo, strictIO, limit }) => {
    ...
    const res = await sanscriptUtilsAPI.post("", {
      query,
      variables: { text, schemeFrom, schemeTo, strictIO, limit },
    });
  ...
  joins: async ({ words, schemeFrom, schemeTo, strictIO }) => {
    ...
    const res = await sanscriptUtilsAPI.post("", {
      query,
      variables: { words, schemeFrom, schemeTo, strictIO },
    });
  ...
  transliterate: async ({ text, schemeFrom, schemeTo }) => {
    ...
    const res = await sanscriptUtilsAPI.post("", {
      query,
      variables: { text, schemeFrom, schemeTo },
    });

...
import useKeyPress, { C_KEY_CTRL } from "./utils/useKeyPressHook";
...
function App() {
  ...
  const { anchorPoint, showContext, selectedText } = useContextMenu({
    showOnlyOnTextSelection: true,
  });

  const drawerStateUpdater = (s) => ({
    ...s,
    drawerOpened: !s.drawerOpened,
    inputText: getSelectionText() || s.inputText,
    inputScheme: _.find(C_TRANSLATE_TEXT_MAP, {
  ...
  const drawerStateUpdaterTrans = (s) => ({
    ...s,
    drawerOpened: !s.drawerOpened,
    fromText: getSelectionText() || s.fromText,
    fromTextLang: _.find(C_TRANSLATE_TEXT_MAP, {
      language: searchParams.get("language") || C_LANGUAGE_DEFAULT,
    })?.value,
  });

...
//   return (
//     <InnerForm {...props} contentData={_.get(data, 'contentLines[0]', {})} refetch={refetch} />
//   )
// }
...
const ContentLineForm = ({ handleClose,
  entityId, parentEntity }) => {
  const { query } = useApolloClient()
  const [languageId, setLanguage] = useState('0')

  const [updateContent, { data: updatedContent }] = useMutation(UPDATE_CONTENT)
  const [deleteContent, { data: deletedContent }] = useMutation(DELETE_CONTENT)
  ...
  useEffect(() => {
    ...
    query({
      ...
      setValue('content', _.get(res, 'data.contentLines[0].content', ''))
      setValue('id', _.get(res, 'data.contentLines[0].id', ''))
      setValue('language', languageId)
    })
  }, [languageId])
...
export default ContentLineForm

...
def get_page_content(driver=None, pageHref=None):
    ...
    postTitle = postTitle.split('–')[-1:][0].strip()
...
def run(url=None, driver_location="/usr/local/bin/chromedriver",
    ...
    try:
        ...
        postTitle = soup.find(
            'h1', attrs={'class': 'post-title entry-title'}).text
        postTitle = postTitle.split('–')[0].strip()
        postCategory = soup.find('li', class_='category').text
        postCategory = postCategory.split('/')[-1:][0]
        postCategory = postCategory.split('-')[0].strip()

        # find content language links
        postContent = soup.find('div', attrs={'class': 'entry-inner'})
        postLanguageLinks = postContent.p.find_all('a')

        postLanguageHref = [a['href'] for a in postLanguageLinks]
        # print(postLanguageHref)
        postLanguageHrefList = []
        for postHref in postLanguageHref:
            reSearch = re.search(r"-in-(\w*)/?$", postHref)
            if reSearch:
                if reSearch.group(1) in languagesDesired:
                    postLanguageHrefList.append(postHref)
            else:
                postLanguageHrefList.append(postHref)

        # print(postLanguageHrefList)
        titleData = {}
        contentData = {}
        categoryData = {}
        for postHref in postLanguageHrefList:
            pageData = get_page_content(driver, postHref)
            contentData[pageData.get('language')] = pageData
            titleData[pageData.get('language')] = {
                'text': pageData.get('title', '')
            }
            categoryData[pageData.get('language')] = {
                'text': pageData.get('category', '')
            }
        ...
        outFile = out_dir + '/' + \
            postTitle.split('–')[0].rstrip().replace(' ', '_') + '.json'
